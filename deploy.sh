...
deploy_cluster() {
    ...
    primaryRunningCount=$( aws ecs describe-services --cluster TCFJCluster --services donorservice --query "services[?serviceArn==`"$SERVICE_ARN"`].[deployments[?status==`PRIMARY`].{runningCount:runningCount}]" --output text)
